...
export default function CompetitionContent() {
  ...
  const signUp = async () => {
    pushRequest("/isSignUp", {username: loginUser, id: id}).then(value => {
      if (value.message[0].isSignUp === true) {
        ...
        pushRequest("/signUpCompetition", { username: loginUser, id: id }).then(value => {
  ...
  return (
    <div>
      <Layout style={{ marginLeft: "13vw", width: "86vw" }} key={id.toString()}>
        <Layout.Content className="layout_content">
          ...
          <div style={{ display: "flex", height: "31vh" }} className="intro">
            <div className="banner">
              <Carousel autoplay>
                <img src={web} alt="web" className="img" key={"1"} />
                <img src={web} alt="web" className="img" key={"2"} />
              </Carousel>
            </div>
